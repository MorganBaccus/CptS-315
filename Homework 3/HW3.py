...
class Perceptron():
    def __init__(self):
        self.traindatafile = 'traindata.txt'
        self.trainlabelfile = 'trainlabels.txt'
        self.testingfile = 'testdata.txt'
        self.testinglabelfile = 'testlabels.txt'
        self.stopwordsfile = 'stoplist.txt'
        self.outputfile = 'output.txt'
        self.learning_rate = 1
        self.num_iterations = 20
        self.num_C = 26
        self.ocrTrain = 'ocr_train.txt'
        self.ocrTest = 'ocr_test.txt'
    ...
    def accuracy(self, weight, examples, labels):
        c = 0
        my_set = np.shape(examples)
        for index in range(0, my_set[0]):
            p_label = np.dot(examples[index], np.transpose(weight))
    ...
    def ft_vectors(self, M, lines, vocabulary):
        fortunes = np.zeros((len(lines), M + 1))
    ...
    def creat_test_vectors(self, M, vocabulary):
        ...
        lines = f.read().split('\n')
        lines.pop()
        test_data = np.zeros((len(lines), M + 1))
    ...
    def cont_t_SP(self, fortunes, max_m, my_set, test_data, test_labels, test_z, train_labels, train_z, w):
        for i in range(1, self.num_iterations + 1):
            mistakes = 0
            for index in range(0, my_set[0]):
                predicted = np.dot(fortunes[index], np.transpose(w))
    ...
    def t_avg_perceptron(self, tot_mistakes, fortunes, test_data, test_labels, train_labels):
        w = np.zeros((1, tot_mistakes + 1))
        u = np.zeros((1, tot_mistakes + 1))
        c = 1
        S = np.shape(fortunes)
        for i in range(1, self.num_iterations + 1):
    ...
    def accuracy2(self, weight, examples, labels, d):
        ...
        for i in range(0, my_set[0]):
            p_label = np.zeros((1, self.num_C))
            for j in range(0, self.num_C):
                p_label[0][j] = np.dot(examples[i], np.transpose(weight[j]))
            if d[np.argmax(p_label)] == labels[i]:
                c += 1
        return c / my_set[0]
    ...
    def train_ft_vectors(self, ocr_train):
        ...
        train_data = np.zeros((len(ocr_train), features + 1))
    ...
    def test_ft_vectors(self, features, ocr_test, ocr_train_label, test_data):
        test_data = np.zeros((len(ocr_test), features + 1))
    ...
    def simpPerceptronHelper(self, num_C, ITERATIONS, LEARNING_RATE, S, el_mistakes, el_test, el_train,
        ...
        for i in range(1, ITERATIONS + 1):
            ...
            for j in range(0, S[0]):
                predicted = np.zeros((1, num_C))
                for k in range(0, num_C):
                    predicted[0][k] = np.dot(train_data[j], np.transpose(w[k]))
                p_index = np.argmax(predicted)
    ...
    def average_perceptron(self, num_C, ITERATIONS, LEARNING_RATE, OUT_FILE, S, el_mistakes, el_test, el_train,
                           features, index_to_letter, letter_to_index, ocr_test_label, ocr_train_label, test_data,
                           train_data):
        w = np.zeros((num_C, features + 1))
        u = np.zeros((num_C, features + 1))
        c = 1
...
def main():
    print("Perceptron being built")
    p = Perceptron()
    p.run()
    print("\n\nPerceptron complete. Please see {0} for results.".format(p.outputfile))
